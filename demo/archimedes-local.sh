#!/bin/bash
# this runs the Archimedes example as a local instace of instance of aca-py
...
PYTHONPATH=.. ../bin/aca-py start \
   ...
   --label archimedes.agent \
   --inbound-transport http 0.0.0.0 8020 \
   --outbound-transport http \
   --admin 0.0.0.0 8021 \
   --admin-insecure-mode \
   --wallet-type indy \
   --wallet-name archimedes.agent916333 \
   --wallet-key archimedes.agent916333 \
   --preserve-exchange-records \
   --auto-provision \
   --genesis-url http://localhost:9000/genesis \
   --trace-target log \
   --trace-tag acapy.events \
   --trace-label archimedes.agent.trace \
